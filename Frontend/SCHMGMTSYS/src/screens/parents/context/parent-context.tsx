...
import { IParentStoreContextType, rootStore, useStore } from "../../../stores";
...
export const ParentContext = createContext<IParentStoreContextType>(
  rootStore.parentStore
);

export const ParentProvider: React.FC<ParentProps> = observer(
  ({ children }) => {
    ...
    const rowClick = async (record: ParentData): Promise<void> => {
      parentStore.setSelectedParent(record);
      await fetchParentById(record.id as number);
    };

    const fetchParentById = useCallback(
      async <TNumber extends number>(parentId: TNumber): Promise<void> => {
    ...
    const searchParentQuery = async <TString extends string>(
      name?: TString
    ): Promise<void> => {
      parentStore.setLoading(true);
      parentStore.setError(null);
